...
function cube_calc_for_surface(
  framebuffer: string[][],
  zBuffer: number[][],
  cubeX: number,
  cubeY: number,
...
function donut_calc_for_surface(
  framebuffer: string[][],
  zBuffer: number[][],
  width: number,
  height: number,
  ...
  const K1 = 50; // sceen distance for scaling

  const x: number = donut_calc_x(
    r1,
    r2,
    sinTheta,
    sinPhi,
  ...
  const z: number =
    donut_calc_z(r1, r2, sinTheta, sinPhi, cosTheta, sinA, cosA, circleX, circleY) +
    distanceFromCamera;
...
export function donut_next_frame(framebuffer: string[][], width: number, height: number) {
  ...
  const thetaSpacing: number = 0.3;
  const phiSpacing: number = 0.3;
...
export function conway_populate(width: number, height: number) {
  ...
  for (let i = 0; i < height; i++) {
    const row: string[] = [];
    for (let j = 0; j < width; j++) {
      const chance: number = Math.floor(Math.random() * (maxFloored - minCeiled) + minCeiled);
      if (chance < 2) {
        row.push(conwayDead);
        continue;
      }
