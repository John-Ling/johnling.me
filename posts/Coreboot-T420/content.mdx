...
**TLDR**: You want UEFI because you'll be able to dual-boot Windows 10/11 and Linux easier. While Windows 10 and 11 can definitely be installed on Legacy BIOS,
to be honest, the world has moved to UEFI. Unless you have very specific reasons to use Legacy, UEFI will be the standard going forward 
...
This guide will only cover the Flash programmer method over the Raspberry PI method. This is because it is the cheaper of the two and the one that I used personally. 
...
In Windows 7 you'll want to download the [BIOS update utility](https://support.lenovo.com/uu/en/downloads/ds018785-bios-update-utility-for-windows-8-32-bit-64-bit-7-32-bit-64-bit-vista-32-bit-64-bit-xp-thinkpad-t420-t420i) and run it. Make sure your device is plugged into power.
...
that at least on the T420 the signal pins are 5V tolerant. I was able to flash Coreboot without modifying my programmer.
...
Attach the clip to the chip. In theory this should be easy. Treat it like a clothes pin and try to get the teeth to make contact with the chip pins. 
...
Attach the clip to the chip following the pinout. Don't worry too much if you put it on wrong, the worst case is that your chip will just heat up, indicating an incorrect orientation. 
...
If that's you, then you're me. My clip was pretty bad and even when I could get it to attach, I was never able to get a successful read.
...
Install the needed dependencies for Flashrom:
Debian-Based:
...
_My laptop used to use an MPE-1800AX WiFi card however any newer version of Coreboot just didn't work with it. If you know you have this WiFi card, you may want to consider using (my config)[https://github.com/John-Ling/coreboot-rom] as it's guaranteed to work._

If you want to build your config yourself make sure you have at least these settings enabled. Using the ? key on each option will give you a bit of information so feel free to explore the config. 
...
_See that circular notch at the top? You want to turn that with a screwdriver but don't apply too much force._
