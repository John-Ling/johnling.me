...
This will, however, cover some weird behaviour and quirks I had to overcome while setting my dashboard up. I'd suggest referring to this article shortly after reading the one I linked if you experience any issues.
...
While DAKboard does provide some scripts to control the display using HDMI CEC. In my experience when I first tried to set it up in 2020, they didn't work. When I revisited the project, I realised that using `xset` to control DPMS (Display Power Management Signalling)
...
I did the opposite to put the display to sleep.
...
Cron is a popular method of running commands at arbitrary times on a Linux system and they're used extensively in this project.

User level Cron jobs are accessed via `crontab -e`

![Cron jobs](/images/blog/Dakboard/image-7.png)

_The @reboot job just runs a boot script (with nothing in it at the moment). We shut the TV off at 9PM and turn it back on at 5AM. Every hour from 5AM to 8PM, we'll reload the display to fix screen blanking issues._

Root level Cron jobs can be accessed via `sudo crontab -e`
...
Due to the dogwater WiFi in my house (thanks Unifi &lt;3), occasionally I would find that my display would not launch the Chromium browser after boot. Looking online, I theorised that the issue was caused by the dashboard failing to connect to WiFi at boot. Since the startup script looked exclusively for a successful (HTTP 200) connection to proceed, the system was left waiting for an internet connection. 

![Screenshot showing that startup code loops waiting for HTTP 200](/images/blog/Dakboard/image-4.png)

To get around this, I simply ran `sudo raspi-config` enabled "Network at Boot" under the "System Options".
...
This was a tough problem since no one really knew what was happening. As reliability is the top priority for the board (doubly so considering I won't be in the country to fix problems if they arise), a dirty but reliable fix was the solution.
...
To prevent the screen from refreshing (and therefore turning back on) at night, I set the Cron job to only operate between 5AM and 8PM.

### Hardware Control

If your TV is like mine, while it does have a standby period after signal is lost, eventually it will go to sleep on its own and not react to new input until the TV is physically turned on.
...
We need this relay to emulate presses of the power button. This meant I had to partially disassemble the TV in order to get access to the power button and solder wires to the contacts. By shorting these wires, I could simulate power button presses. 
...
After soldering my wires, I attached them to the relay's COM (common) and NO (normally open) terminals respectively. I attached jumper wires to the 3.3V, ground and PIN 14 GPIO pins on the Pi. After that, I could control them very easily using this script.
