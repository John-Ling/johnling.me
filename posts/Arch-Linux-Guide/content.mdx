...
Hit ENTER and you'll be placed into the live ISO environment. 
...
Now that we have divided our disk into partitions, we must next format them and create their own file systems.
...
_BTRFS allows for snapshots meaning you can rollback to a previous good state if an update breaks your system. Additionally, features such as transparent file compression which can increase usable disk space. XFS is common on enterprise servers but is also popular on Gentoo and is the filesystem recommended by the install guide._

_Arch's benefit comes in allowing you that level of control of your operating system all the way down to the filesystem level. While this guide does not cover BTRFS since I personally do not use it and as such know as much as you do, resources online are abundant._
...
_The pacstrap command simply an alias for the pacman command `pacman -r /mnt -Sy --cachedir=/mnt/var/cache/pacman/pkg --noconfirm`. Here we are telling pacman to install the package base (suite of basic packages for Arch), linux and linux-firmware to /mnt directory (where we've mounted our root partition)._
...
_Bootloaders come in 2 types first-stage and second-stage. Bootloaders like **GRUB or systemd-boot are second-stage bootloaders**. First-stage bootloaders refer to your BIOS or UEFI._
