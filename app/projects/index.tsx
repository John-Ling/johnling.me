...
export default function ProjectsPage() {
	...
	return (
		<>
			<AnimatePresence>
				{modalOpened && selectedProject !== undefined && <ProjectModal project={selectedProject} on_close={onClose} />}
			</AnimatePresence>
			<title>Projects</title>
			<div className={`min-h-screen w-11/12 lg:w-10/12 2xl:w-10/12 mx-auto max-w-[1920px] pt-10`}>
				<motion.h1
					...
					{projects.map((project: Project, i: number) => {
						return (
							<motion.span variants={item} key={i}>
								<ProjectListCard key={i} project={project} position={i} cols={cols} on_select={onProjectSelect} />
							</motion.span>
						);
					})}
				</motion.div>
